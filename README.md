...
一句话定位：**把任意网页以「全屏、锁定、可远程集中管理」的方式跑在无人值守的公共显示终端上**——数字标牌 / 信息屏专用的 Kiosk 浏览器 + 配套的远程配置服务端。
...
| | 浏览器直接打开 | 本项目 |
|---|---|---|
| 全屏锁定 | ✗ 可关闭/切走 | ✓ 强制独占 + 密码保护 |
| 多设备管理 | ✗ | ✓ 注册/分组/远程下发/审计 |
| 无人值守 | 弱 | ✓ 断网恢复、防烧屏、自动刷新 |
| 远程运维 | ✗ | ✓ 远程改密、配置回滚 |
...
> 部署提示：目标机器需安装 **WebView2 Evergreen Runtime**（客户端不内置运行时）；产物为单文件 exe，可直接拷贝运行。
